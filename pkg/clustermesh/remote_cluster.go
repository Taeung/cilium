	...
	rc.controllers.UpdateController(rc.remoteConnectionControllerName,
		controller.ControllerParams{
			DoFunc: func() error {
				...
				go ipCacheWatcher.Watch([]ipcache.IPIdentityMappingListener{rc})
...
func (rc *remoteCluster) OnIPIdentityCacheChange(modType ipcache.CacheModification, oldIPIDPair *identity.IPIdentityPair, newIPIDPair identity.IPIdentityPair) {
	if newIPIDPair.ID.ClusterID() == option.Config.ClusterID {
		rc.getLogger().WithField("identity", newIPIDPair).
			Warning("Received ipcache entry for own cluster ID")
		return
	}

	rc.mesh.conf.IdentityChangeHandler(modType, oldIPIDPair, newIPIDPair)
}

func (rc *remoteCluster) OnIPIdentityCacheGC() {
	// No garbage collection needs to be done for remote cluster
}
